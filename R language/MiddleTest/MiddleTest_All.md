...
| 항목 | POSIXlt | POSIXct |
|------|------|------|
🔧 구조 | 리스트(list) 형태 | 숫자형(numeric) 형태 |
📆 기준 | 년, 월, 일, 시, 분, 초 등의 구조화된 정보 제공 | 1970-01-01 00:00:00 UTC부터의 초 단위 시간 |
🔍 접근성 | $year, $mon 등으로 세부 정보 접근 가능 | 불가능 (숫자 기반이라 직접 접근 안 됨) |
💾 메모리 | 더 많은 메모리 사용 | 적은 메모리 사용 |
⚡ 속도 | 느림 | 빠름 |
🧑‍💻 용도 | 날짜 구성 요소를 나눠서 다룰 때 | 계산/정렬 등 시간 차 연산이 주 목적일 때 |
